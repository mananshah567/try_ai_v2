...
# --------------------------------------
# Load Existing Vertices and Edges
# --------------------------------------
def load_existing_graph(client, throttler):
    ...
    try:
        query = "g.E().project('out', 'label', 'in').by(outV().id()).by(label()).by(inV().id())"
        result_set = client.submit(query)
...
# --------------------------------------
def insert_vertices_by_batch(client, df_vertices, throttler):
    for i, row in df_vertices.iterrows():
        vertex_id = str(row['ID'])
        label = str(row['Label'])
        query = f"g.addV('{label}').property('id', '{vertex_id}')"
        ...
        try:
            ...
            logger.error(f"Insert vertex {vertex_id} failed: {e}")

# --------------------------------------
# Batch Insert Edges
# --------------------------------------
def insert_edges_by_batch(client, df_edges, throttler):
    for i, row in df_edges.iterrows():
        out_id = str(row["OutV"])
        in_id = str(row["InV"])
        label = str(row["Label"])
        ...
        try:
            ...
            logger.error(f"Insert edge {out_id}-{label}->{in_id} failed: {e}")
