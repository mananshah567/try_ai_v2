import time
import itertools
from gremlin_python.driver.client import Client
from ipywidgets import IntProgress, HTML, VBox
from IPython.display import display

def chunked(iterable, size):
    """Yield successive `size`‐sized chunks from `iterable`."""
    it = iter(iterable)
    while True:
        chunk = list(itertools.islice(it, size))
        if not chunk:
            return
        yield chunk

def load_edges_in_vertex_batches_option_b(
    client: Client,
    throttler,
    vertex_ids,
    vertex_batch_size: int = 1_000,
    edge_page_size:   int = 500
):
    """
    Load all existing edges by batching vertices, but look up each batch via
    `.has('id', within(...))` instead of composite-key V([...]).
    Returns a set of composite keys "out_label_in".
    """
    inserted = set()
    total_vertices = len(vertex_ids)

    # progress bar over total vertices
    v_label = HTML()
    v_bar   = IntProgress(min=0, max=total_vertices, description="Vertices:")
    display(VBox([v_label, v_bar]))

    processed = 0
    start = time.time()

    # Gremlin template: we’ll fill in {ids_list} with a literal like "'v1','v2','v3',…"
    gremlin_tpl = """
      g.V()
       .has('id', within({ids_list}))
       .bothE()
       .project('out','label','in')
         .by(outV().id())
         .by(label())
         .by(inV().id())
    """

    for batch in chunked(vertex_ids, vertex_batch_size):
        # build the literal list of quoted IDs
        ids_list = ",".join(f"'{vid}'" for vid in batch)
        gremlin = gremlin_tpl.format(ids_list=ids_list)
        continuation = None

        # page through all edges for this batch
        while True:
            rs = client.submit(
                gremlin,
                {},
                request_options={"continuation": continuation, "pageSize": edge_page_size}
            )
            edges = rs.all().result()
            attrs = rs.status_attributes

            # throttle on RU
            ru = float(attrs.get("x-ms-total-request-charge", 10.0))
            throttler.throttle(ru)

            # collect each edge
            for e in edges:
                inserted.add(f"{e['out']}_{e['label']}_{e['in']}")

            continuation = attrs.get("x-ms-continuation")
            if not continuation:
                break

        # update outer progress
        processed += len(batch)
        v_bar.value = processed
        elapsed = time.time() - start
        rate = processed / elapsed if elapsed else 0
        eta  = (total_vertices - processed) / rate if rate else float('inf')
        v_label.value = (
            f"Processed {processed}/{total_vertices} vertices in {elapsed:.1f}s; "
            f"ETA {eta:.1f}s"
        )

    return inserted

# Usage:
# inserted_edge_keys = load_edges_in_vertex_batches_option_b(
#     client, throttler,
#     vertex_ids=inserted_vertex_ids,
#     vertex_batch_size=1000,
#     edge_page_size=500
# )
