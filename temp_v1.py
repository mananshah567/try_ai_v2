...
def upload_vertices_checked(client, df_vertices, throttler):
    ...
    for i, row in df_vertices.iterrows():
        ...
        check_query = f"g.V('{vertex_id}').hasLabel('{label}').limit(1)"
        try:
            check_result = client.submit(check_query).all().result()
            if check_result:
                continue
            throttler.throttle(10)
        except Exception as e:
            logger.error(f"Vertex existence check failed for {vertex_id}: {e}")
            continue
...
def upload_edges_checked(client, df_edges, throttler):
    ...
    for i, row in df_edges.iterrows():
        ...
        check_query = f"g.V('{out_id}').outE('{label}').where(inV().hasId('{in_id}')).limit(1)"
        try:
            check_result = client.submit(check_query).all().result()
            if check_result:
                continue
            ...
            logger.error(f"Edge existence check failed for {out_id}-{label}->{in_id}: {e}")
            continue
        ...
        try:
            ...
            logger.error(f"Edge error {out_id}-{label}->{in_id}: {e}")
